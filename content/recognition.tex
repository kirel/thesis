...
In dieser Form hat DTW die Laufzeitkomplexität \(\mathcal(O)(n*m)\). Darum wundert es nicht, dass bereits einige Wissenschaftler versucht haben, den Algorithmus zu optimieren und schneller zu machen. Auch für Detexify ist dieses Laufzeitverhalten nicht ausreichend um eine Erkennung in Echtzeit zu ermöglichen.

% subsubsection der_klassische_dtw_algorithmus (end)

\subsubsection{constrained warping window} % (fold)
...
\subsubsection{lower bounding} % (fold)
